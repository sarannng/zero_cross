...
class GameMp1 extends StatefulWidget {
  final String docid1;

  const GameMp1({Key key, this.docid1}) : super(key: key);

  _GameMp1State createState() => _GameMp1State();
}

class _GameMp1State extends State<GameMp1> {
  bool chance;
  bool turn;
  String marker;
  String one;
  ...
  createristrue(creater, joiner){
    ...
    print('Createistrue called');
     ...
     else if(!joiner && !creater && one!='-' && one!=null
     && two!='-' && two!=null
     && three!='-' && three!=null
     && four!='-' && four!=null
     && five!='-' && five!=null
  ...
  check() {
    if ((one == 'X' && two == 'X' && three == 'X' ||
      ...
      // return alert;
 Firestore.instance.collection('rooms').document('${widget.docid1}').updateData({ 
        'joiner': true,
      });
    // if(joiner){
    //     showDialog(
